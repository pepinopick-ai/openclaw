...
function _saveReviewHistory(history) {
  try {
    ...
    if (keys.length > REVIEW_HISTORY_MAX_ENTRIES) {
      const sorted = keys.map((k) => ({ k, t: history[k].at || 0 })).sort((a, b) => a.t - b.t);
...
function _buildReviewContent({ outcome, gene, signals, blast, sourceType }) {
  ...
  const score =
    outcome && Number.isFinite(Number(outcome.score)) ? Number(outcome.score).toFixed(2) : "?";
  ...
  if (blast) {
    parts.push(
      "Blast radius: " + (blast.files || 0) + " file(s), " + (blast.lines || 0) + " line(s)",
    );
  }
